...
add_action('init', function() {
    ...
    $posted_method = isset($_POST['method']) ? sanitize_text_field(wp_unslash($_POST['method'])) : '';
    $posted_pod = isset($_POST['_pods_pod']) ? sanitize_key(wp_unslash($_POST['_pods_pod'])) : '';
    $posted_id = isset($_POST['_pods_id']) ? trim((string) wp_unslash($_POST['_pods_id'])) : '';
    $posted_uri = isset($_POST['_pods_uri']) ? sanitize_text_field(wp_unslash($_POST['_pods_uri'])) : '';
    $posted_form = isset($_POST['_pods_form']) ? sanitize_text_field(wp_unslash($_POST['_pods_form'])) : '';
    $posted_nonce = isset($_POST['_pods_nonce']) ? sanitize_text_field(wp_unslash($_POST['_pods_nonce'])) : '';

    $pods_uid = '';
    if (is_user_logged_in()) {
        $pods_uid = 'user_' . get_current_user_id();
    } elseif (function_exists('pods_session_id')) {
        $pods_uid = (string) pods_session_id();
    }

    $pods_action = '';
    $pods_nonce_valid = null;
    if ($posted_nonce !== '' && $posted_pod !== '' && $posted_uri !== '' && $posted_form !== '' && $pods_uid !== '') {
        $field_hash = wp_create_nonce('pods_fields_' . $posted_form);
        $pods_action = 'pods_form_' . $posted_pod . '_' . $pods_uid . '_' . $posted_id . '_' . $posted_uri . '_' . $field_hash;
        $pods_nonce_valid = (false !== wp_verify_nonce($posted_nonce, $pods_action));
    }

    $cookie_session = isset($_COOKIE['PHPSESSID']) ? (string) $_COOKIE['PHPSESSID'] : '';
    ...
    meldetool_debug_log('TEAM_FORM_POST_RECEIVED', array(
        ...
        'pods_id'             => $posted_id,
        'pods_uri'            => $posted_uri,
        'pods_form'           => $posted_form,
        'nonce_set'           => ($posted_nonce !== '') ? 'yes' : 'no',
        'nonce_valid'         => ($pods_nonce_valid === null) ? 'unknown-missing-context' : ($pods_nonce_valid ? 'VALID' : 'INVALID-OR-EXPIRED'),
        'pods_uid'            => $pods_uid,
        'has_action'          => ($pods_action !== '') ? 'yes' : 'no',
        'cookie_phpsessid'    => ($cookie_session !== '') ? substr($cookie_session, 0, 8) . '...' : 'missing',
        'cookie_count'        => isset($_COOKIE) && is_array($_COOKIE) ? count($_COOKIE) : 0,
        'is_doing_ajax'       => (defined('DOING_AJAX') && DOING_AJAX) ? 'yes' : 'no',
