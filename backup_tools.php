...
function meldetool_backup_collect_posts($post_type, $taxonomies) {
    ...
    foreach ($posts as $post) {
        ...
        foreach ($meta as $meta_key => $values) {
            if (in_array($meta_key, array('_edit_lock', '_edit_last', '_wp_old_slug', '_wp_trash_meta_status', '_wp_trash_meta_time'), true)) {
                continue;
            }

            if (strpos($meta_key, '_uag_') === 0 || strpos($meta_key, '_uagb_') === 0) {
                continue;
            }
